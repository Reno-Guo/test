            ...
            try:
                ...
                for col1, col2 in data_rows:
                    ...
                    if not is_b0_pattern:
                        ...
                        if "auto" in col2:
                            label = "Auto KW" if label == "Brand PAT" else "Auto PAT"
...
with st.sidebar:
    st.markdown("### 📖 使用说明")
    st.markdown('<div class="info-box"><b>操作步骤：</b><br>1️⃣ 上传包含数据的 Excel 文件（可多个）<br>2️⃣ 上传匹配 ASIN 的 Excel 文件<br>3️⃣ 点击"开始处理"按钮<br>4️⃣ 等待处理完成并下载结果</div>', unsafe_allow_html=True)
    
    st.markdown("### 📋 文件格式要求")
    with st.expander("📁 数据文件格式"):
        st.markdown("**文件类型**: `.xlsx`\n\n**列结构**:\n- 第1列: Query（关键词/ASIN）\n- 第5列: Campaign Type\n\n**示例**:\n```\n| Query      | Campaign Type |\n| oneplus 12 | Manual        |\n| B09XYZ1234 | Auto          |\n```")
    
    with st.expander("🔍 匹配文件格式"):
...
with col1:
    st.markdown("### 📁 数据文件")
    st.markdown('<div style="background-color: #f0f9ff; padding: 10px; border-radius: 5px; margin-bottom: 10px;"><small><b>格式要求：</b><br>• 文件格式：<code>.xlsx</code><br>• 第1列：Query（关键词/ASIN）<br>• 第5列：Campaign Type</small></div>', unsafe_allow_html=True)
...
with col2:
    st.markdown("### 🔍 匹配文件")
    st.markdown('<div style="background-color: #fff5e6; padding: 10px; border-radius: 5px; margin-bottom: 10px;"><small><b>格式要求：</b><br>• 文件格式：<code>.xlsx</code><br>• 第1列：ASIN 列表<br>• 用于判断 Brand PAT 和 CMP PAT</small></div>', unsafe_allow_html=True)
    match_file = st.file_uploader("选择包含 ASIN 的匹配文件（单个）", type=['xlsx'], key="match_file")
    if match_file:
